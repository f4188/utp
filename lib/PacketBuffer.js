...
//buffer initialized and empty, startseq, maxseq, length = null
//buffer with packets , startseq, maxseq, length integers, maxseq-startseq % Math.pow(2,16) = length % Math.pow(2,16)
// if length > Math.pow(2,16) overwrite, throw exception
//buffer empty

function PacketBuffer (maxBytes) {
	this.buffer = []
	this.startSeq = null; //smallest seq
	this.maxSeq = null
	this.num = null
	this.bytes = 0
	this.maxBytes =  maxBytes || 1500 * 1200 //largest seq
